<script>
    ...
    let grid = null; // Single GridStack instance for the container
  
    // Initialize GridStack on the main container
    function initializeGrid() {
      const container = document.querySelector('#main-container');
      if (!container) {
        console.error('Container #main-container not found');
        return;
      }
      if (!window.GridStack) {
      ...
      try {
        grid = window.GridStack.init({
          ...
          column: 12,
          resizable: {
            handles: 'se', // Only bottom-right handle
            autoHide: false // Prevent auto-hiding
          },
          margin: 10,
          alwaysShowResizeHandle: true // Always show resize handle
        }, '#main-container');
  
        // Update local dataText store on grid change
        ...
        // Load initial items into GridStack
        const texts = $dataText;
        if (grid && texts.length) {
          console.log('Loading initial widgets:', texts);
          grid.load(texts.map(text => ({
            id: `object-${text.id}`,
            x: text.x || 0,
            y: text.y || 0,
            w: text.w || 12,
      ...
      return () => {
        if (grid) {
          try {
            ...
            grid.destroy(true); // Destroy GridStack and remove DOM nodes
          } catch (error) {
    ...
    function deleteText(id) {
      if (confirm('Bạn có chắc chắn muốn xóa object này?')) {
          ...
          if (el) {
            try {
              ...
              if (el.parentNode) {
                ...
                console.log('Widget removed manually from DOM');
              }
            }
          } else {
        ...
        // Update store after removing widget
        dataText.update(current => {
          const newCurrent = current.filter(text => text.id !== id);
          console.log('Updated dataText:', newCurrent);
          return newCurrent;
        });
        // Log remaining grid-stack-items
      ...
      if (updatedText.id) {
        ...
        if (grid) {
          try {
            ...
            console.log('Widget added to GridStack:', newId);
          } catch (error) {
...
;
    ...
    :global(.grid-stack-item:not(.object-container)) {
      pointer-events: none;
      display: flex;
      flex-direction: column;
      align-items: center;
    ...
    :global(.grid-stack-item:not(.object-container) .grid-stack-item-content) {
        pointer-events: auto;
    height: 0.4rem;
    margin: 0.3rem;
    ...
    :global(.grid-stack-item:not(.object-container) .grid-stack-item-content):hover {
      background-color: #43646d;
      width: 50%;
    }
